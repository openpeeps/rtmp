...
var gStaging = initTable[int, seq[byte]]()  # connId -> staging buffer for partial headers
  # Keeps ConnCtx refs alive: conn.data is a raw pointer invisible to the GC,
  # so without this table the ref would be collected right after onClientAccept.
var gConnCtxAlive = initTable[int, ConnCtx]()
...
proc onClientData(conn: Connection, data: openArray[byte]) =
  ...
  let basePtr = if hasStaging: cast[ptr UncheckedArray[byte]](addr feedData[0])
                else: cast[ptr UncheckedArray[byte]](unsafeAddr data[0])
  let totalLen = if hasStaging: feedData.len
                 else: avail

  proc stageTail(pos: int) =
    ## Persist unconsumed bytes (from pos onward) for the next read.
    let rem = totalLen - pos
    if rem <= 0:
      return
    gStaging[c.connId] = newSeq[byte](rem)
    copyMem(addr gStaging[c.connId][0],
            cast[ptr UncheckedArray[byte]](cast[uint](basePtr) + pos.uint), rem)

  # Handshake states consume fixed-size prefixes from the front of the view;
  # an explicit cursor keeps post-handshake bytes intact across transitions.
  var pos = 0

  if c.hsState == HS_INIT:
    if totalLen - pos < 1 + RTMP_HANDSHAKE_SIZE:
      stageTail(pos)
      return

    let pbytes = cast[ptr UncheckedArray[byte]](cast[uint](basePtr) + pos.uint)
    let c0 = pbytes[0]
    if c0 != 0x03'u8:
      conn.close()
      return
    ...
    pos += 1 + RTMP_HANDSHAKE_SIZE
    c.hsState = HS_S0S1_SENT

  if c.hsState == HS_S0S1_SENT:
    if totalLen - pos < RTMP_HANDSHAKE_SIZE:
      stageTail(pos)
      return

    # Validate C2 equals our S1 when we sent a plain handshake (best-effort).
    let c2bytes = cast[ptr UncheckedArray[byte]](cast[uint](basePtr) + pos.uint)
    ...
    pos += RTMP_HANDSHAKE_SIZE
    c.hsState = HS_DONE

  # HS_DONE: feed RTMP chunks; keep any unconsumed tail staged
  let consumed = feedBytes(c.chunkCtx,
    cast[ptr byte](cast[uint](basePtr) + pos.uint), totalLen - pos)
  if consumed <= 0:
    return
  pos += consumed
  stageTail(pos)
...
proc apiRequestHandler*(req: HttpRequest, res: HttpResponse) {.gcsafe.} =
  ...
  var body: string
  {.cast(gcsafe).}:
    # Single-threaded event loop: the monitor is only mutated from this loop,
    # so reading it from an HTTP handler on the same loop needs no locking.
    # Direct typed access is required — routing the ref through pointer casts
    # corrupts serialization under ORC.
...
proc onClientAccept(conn: Connection) =
  ...
  gConnCtxAlive[local.connId] = local
