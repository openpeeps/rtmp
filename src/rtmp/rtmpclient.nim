...
proc pushNextAacFrame(client: RtmpClient): bool =
  ...
  # Backpressure: skip if a file transfer is active
  if client.conn.sendFileFd >= 0:
    return
  ...
  # Zero-copy file streaming via sendFileFd
  client.conn.sendFileFd = a.fd
  client.conn.sendFileOff = a.pos + headerLen
  client.conn.sendFileRemain = rawLen
  if not client.conn.continueSendFile():
    discard  # EAGAIN: pacer timer will retry
...
proc pushNextFlvTag(client: RtmpClient): bool =
  ...
  # Backpressure: skip if a file transfer is active
  if client.conn.sendFileFd >= 0: return
  ...
  # Zero-copy file streaming via sendFileFd
  client.conn.sendFileFd = st.fd
  client.conn.sendFileOff = st.tagPayloadPos
  client.conn.sendFileRemain = toSend
  if not client.conn.continueSendFile():
    discard  # EAGAIN: pacer timer will retry
...
proc scheduleSend(client: RtmpClient, delayMs: int) =
  ...
  client.sendTimerId = client.loop.addTimer(d) do (id: int):
    ...
    if nextTs >= 0:
      ...
      # Check for stream completion: all queued data drained
      let sendfileActive = clientAddr.conn.sendFileFd >= 0
      let outputDrained = clientAddr.conn.flushWriteBuffer()
      if (clientAddr.stream == nil or clientAddr.stream.done) and
         (clientAddr.aac == nil or clientAddr.aac.done) and
         outputDrained and not sendfileActive:
        if clientAddr.stream != nil:
